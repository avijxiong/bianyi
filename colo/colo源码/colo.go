...
func main() {
	...
	// 添加新的处理步骤
	if *ipsType == "4" {
		err := processAndSaveIPv4Subnets(*outFile)
		if err != nil {
			fmt.Printf("处理IPv4子网时出错: %v\n", err)
		} else {
			fmt.Println("已生成更新后的 ips-v4.txt 文件")
		}
	}
}
...
// 新增函数：处理CSV文件并生成IPv4子网列表
func processAndSaveIPv4Subnets(csvFile string) error {
	// 读取CSV文件
	...
	if err != nil {
		return fmt.Errorf("读取CSV标题时出错: %v", err)
	}
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("读取CSV记录时出错: %v", err)
		}

		ip := record[0] // 假设IP地址在第一列
		if strings.Contains(record[4], "ms") { // 检查是否包含 "ms"
	...
	// 将子网写入新文件
	outFile, err := os.Create("ips-v4.txt")
	if err != nil {
	...
	for subnet := range subnets {
		_, err := outFile.WriteString(subnet + "\n")
		if err != nil {
			return fmt.Errorf("写入子网时出错: %v", err)
		}
	}
